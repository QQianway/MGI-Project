...
$target_dir = "../../data/genesequenceAA/";
...
if ($uploadOk !=0)
{
	...
	foreach ($pieces as $piece) {
		if ($piece != "")
		{
			...
			$sql = "insert into geneseq(geneSeqName,geneSeq,type) values('$lines[0]','$seq','1')";
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file))
	{
		...
		$makeDB="./makeblastdb ";
		$input="-in ../../data/genesequenceAA/".$genomeName.".predictedgene.faa ";
		$output="-out ../db/".$genomeName."FAA ";
		$dbtype="-dbtype prot ";
		...
		while($getAllGenomeNameRows=mysqli_fetch_array($getAllGenomeName)){
			...
			$allDB=$allDB."../db/".$allgenomeName."FAA ";
		}
		$mergeDB="./blastdb_aliastool ";
		$listDB="-dblist \"".$allDB."\" ";
		$outMergeDB="-out ../db/FAA ";
		$title="-title \"Amino Acid Seqeunces Database\"";
		$mergeCommand=$mergeDB.$listDB.$outMergeDB.$dbtype.$title;
		$MERGECOMMAND=escapeshellcmd($mergeCommand);
		exec ($MERGECOMMAND);
		$_SESSION['UPLOAD']=3;
        echo "<script>uploadSuccess3()</script>";
    } 
	else 
	{
        echo "<script>uploadFailed()</script>";
