...
if ($uploadOk!=0)
{
	...
	while(!feof($myfile)) 
	{
		 ...
		 if ($entry[0]!='#')
		{
			 ...
			 if ($columns[0]!='')
			 {
				 $sql = "INSERT INTO `trna`(`tRNAID`, `scaffoldID`, `source`, `feature`, `start`, `end`, `score`, `strand`, `frame`, `attribute`) VALUES ('','$columns[0]','$columns[1]','$columns[2]','$columns[3]','$columns[4]','$columns[5]','$columns[6]','$columns[7]','$columns[8]')";
...
// if everything is ok, try to upload file
	...
	{
	$jbrowseCommand="../../jbrowse/bin/flatfile-to-json.pl --gff ".$target_file." --trackLabel tRNA --out ../../jbrowse/data/".$row['genomeName']."/";
	$jbrowseCOMMAND=escapeshellcmd($jbrowseCommand);
	exec($jbrowseCOMMAND); 
	$_SESSION['UPLOAD']=7; 
        echo "<script>uploadSuccess7()</script>";
    } 
	else 
	{
        echo "<script>uploadFailed()</script>";
