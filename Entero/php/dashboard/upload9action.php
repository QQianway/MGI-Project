...
if ($uploadOk!=0)
{
	...
	while(!feof($myfile)) 
	{
		...
		if ($entry[0]!='#')
		{
			 ...
			 if ($columns[0]!='')
			 {
				  $sql = "INSERT INTO `rrna`(`rRNAID`, `scaffoldID`, `source`, `feature`, `start`, `end`, `score`, `strand`, `frame`, `attribute`) VALUES ('','$columns[0]','$columns[1]','$columns[2]','$columns[3]','$columns[4]','$columns[5]','$columns[6]','$columns[7]','$columns[8]')";
...
// if everything is ok, try to upload file
	...
	$target_file = $target_dir . basename($row['genomeName'].'.rrna.'.$FileType);
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file))
	{
	$jbrowseCommand="../../jbrowse/bin/flatfile-to-json.pl --gff ".$target_file." --trackLabel rRNA --out ../../jbrowse/data/".$row['genomeName']."/";
	$jbrowseCOMMAND=escapeshellcmd($jbrowseCommand);
	exec($jbrowseCOMMAND); 
	$_SESSION['UPLOAD']=9;       
	echo "<script>uploadSuccess9()</script>";	
    } 
	else 
	{
        echo "<script>uploadFailed()</script>";
